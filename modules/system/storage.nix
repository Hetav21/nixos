...
in {
  options.system.storage = {
    enable = mkEnableOption "Enable CLI/TUI storage tools (rclone, syncthing)";
    enableGui = mkEnableOption "Enable GUI storage tools (localsend, megasync, onedrive)";
  };

  config = mkMerge [
    # CLI/TUI storage tools
    (mkIf cfg.enable {
