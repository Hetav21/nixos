...
- **Packages**: AI agent CLIs (e.g. `claude-code`, `codex`, `coderabbit-cli`) come from `pkgs.llm-agents.*` via the `llm-agents.overlays.shared-nixpkgs` overlay (binary cache hits when nixpkgs revision matches upstream).
