...
- **Ship Protocol (PR & Merge Gate)**: Once changes and verification are complete, the agent presents an interactive release gate (`ask_question` or keyword `ship`). Upon approval, the agent autonomously commits, pushes, creates the PR, triggers & addresses CodeRabbit reviews, merges, and pulls fresh `main`. See [AGENTS.md](../AGENTS.md#release-gate-protocol-ship-workflow).
